...
const fmtCOP=n=>isFinite(n)?n.toLocaleString("es-CO",{style:"currency",currency:"COP",maximumFractionDigits:0}):"—";
const fmtNum=n=>isFinite(n)?n.toLocaleString("es-CO",{maximumFractionDigits:2}):"—";
...
function buildDictamen({year,pcl,sml,smlDv,cop,isPremium,user,plan,daysLeft}){
  const head=`DICTAMEN PRELIMINAR (— NO CONSTITUYE ASESORÍA JURÍDICA)\n`;
  const base=`Año base SMMLV: ${year}\nPCL reportada: ${isFinite(pcl)?pcl:"—"}%\nEquivalencia aplicada (SMLDV): ${fmtNum(sml)}\nValor SMLDV (COP): ${fmtCOP(smlDv)}\nIndemnización estimada (COP): ${fmtCOP(cop)}\n\n`;
  const norm=`Criterio de cálculo: regla por tramos con tope de 180 SMLDV para IP (SOAT).\n\n`;
...
async function refreshPremiumFromStored(){
  ...
  premium=res.ok?res:{ok:false,reason:res.reason||"Licencia inválida"};
  return premium;
}
function renderPremiumUI(){
  ...
  if(premium.ok){
    ...
    d.textContent="Días restantes: "+String(premium.daysLeft);
  }else{
...
function applyPremiumGates(dictText){
  ...
  if(!premium.ok){
    dict.value="🔒 Dictamen preliminar disponible solo en versión PREMIUM.";
...
function computeAndRender(){
  ...
  $("kpiSml").textContent=isFinite(eq)?fmtNum(eq):"—";
  $("kpiSmlDv").textContent=isFinite(smldv)?fmtCOP(smldv):"—";
  $("kpiCop").textContent=isFinite(cop)?fmtCOP(cop):"—";
...
async function init(){
  ...
  $("btnValidateLicense").addEventListener("click",async()=>{
    ...
    const res=await window.PCLSOAT_LICENSE.verifyToken(token);
    if(!res.ok){setMsg($("licenseMsg"),res.reason||"Licencia inválida.",false);return;}
    window.PCLSOAT_LICENSE.saveToken(token);
    premium=res;
    renderPremiumUI();
    computeAndRender();
    setMsg($("licenseMsg"),"Licencia válida. PREMIUM activo.",true);
  });
